<template>
    <div class="pagination">
      ...
      <button v-if="startNumAndEndNum[startNumAndEndNum.length-1]!=totalPage" @click="$emit('getPageNo',totalPage)" :class="{active:pageNo==totalPage}"> {{totalPage}}</button>
      ...
      methods: {
       ...
       computed:{
        ...
        startNumAndEndNum(){
            ...
            // 先定义两个变量存储起始数字与结束数字
            let start=0,end=0;
            // 联系页码数字5【就是至少五页】，如果出现不正常的现象【就是不够五页】
            // 不正常现象【总页数没有联系页码多】
